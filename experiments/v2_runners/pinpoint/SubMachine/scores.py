"""
[작성일] 24.06.04 16:38
- IPIRec (AdjustedBiasedCorrelationPinpointEstimator): TEST & OBS.
    - Tags score: Adjusted, Biased
    - Feedback
        - train()
            - FOREACH (DecisionsTypeList a) THEN { AdjustScore(a); Personalization(a); }
        - DTypeSeq: VLP
"""
...
WORKSPACE_HOME = os.path.dirname(__file__).replace(
    f"/experiments/pinpoint/{__dir_name__}", ""
)
""".../ipirec"""
DATA_SET_HOME = f"{WORKSPACE_HOME}/data"
""">>> `${WORKSPACE_HOME}`/data"""
MODEL_NAME = "Scores"
RESULTS_SUMMARY_HOME = f"{WORKSPACE_HOME}/results/pinpoint/vlp"
""">>> `${WORKSPACE_HOME}`/results/pinpoint/vlp"""
...
def build_models(
    ...
    for k in range(KFold):
        ...
        model.analysis()
        estimator_params = (
            AdjustedBiasedCorrelationPinpointEstimator.create_models_parameters(
        ...
        estimator = AdjustedBiasedCorrelationPinpointEstimator(
            model=model,
...
if __name__ == "__main__":
    ### [24.06.04] Redo >> AdjustedBiasedCorrelationPinpointEstimator
    ## Common Args
    top_n_tags_list = [20]
    score_iter_list = [3]
    score_learing_rate_list = [0.001]
    score_generalization_list = [0.00001]
    weight_iter_list = [5]
    ...
    # Scores
    score_iter_list = [3, 5, 10, 20, 30]
    score_learing_rate_list = [10 ** (-1 * n) for n in range(2, 6, 1)]
    score_generalization_ratio_list = [0.1, 0.001]
    # score_generalization_list = [0.00001]
            ...
            for co_occur_items in co_occur_items_list:
                for score_iterations in score_iter_list:
                    for score_learning_rate in score_learing_rate_list:
                        for score_ratio in score_generalization_ratio_list:
                            score_generalization = score_learning_rate * score_ratio
