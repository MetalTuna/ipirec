...
WORKSPACE_HOME = os.path.dirname(__file__).replace(
    f"/experiments/rev_240602_1527/vlp/{__dir_name__}", ""
)
""".../ipirec"""
DATA_SET_HOME = f"{WORKSPACE_HOME}/data"
""">>> `${WORKSPACE_HOME}`/data"""
MODEL_NAME = "IPIRecAB_Scores"
RESULTS_SUMMARY_HOME = f"{WORKSPACE_HOME}/results/params/rev_240603/vlp_SubMachine"
""">>> `${WORKSPACE_HOME}`/results/params/rev_240603/vlp_SubMachine"""
...
def build_models(
    ...
    for k in range(KFold):
        ...
        model.analysis()
        estimator_params = (
            AdjustedBiasedCorrelationEstimatorRev3.create_models_parameters(
        ...
        estimator = AdjustedBiasedCorrelationEstimatorRev3(
            model=model,
        ...
        for decision_type in [
            ...
            estimator._is_preprocessed = False
            estimator.train(
...
if __name__ == "__main__":
    ### [24.06.03] Obs. >> tags_score - SubMachine
    ## Opt. Rngs.
    top_n_tags_list = [10]
    score_iter_list = [n for n in range(40, 80, 10)]
    score_learing_rate_list = [10 ** (-1 * n) for n in range(7, 10, 1)]
    score_feedback_ratio = 1 / 10
                ...
                for score_iterations in score_iter_list:
                    for score_learning_rate in score_learing_rate_list:
                        score_generalization = (
                            score_learning_rate * score_feedback_ratio
                        )
