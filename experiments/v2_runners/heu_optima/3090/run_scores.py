...
WORKSPACE_HOME = os.path.dirname(__file__).replace(
    f"/experiments/heu_optima/{__dir_name__}", ""
)
""".../ipirec"""
DATA_SET_HOME = f"{WORKSPACE_HOME}/data"
""">>> `${WORKSPACE_HOME}`/data"""
MODEL_NAME = "IPIRecAB_Scores"
RESULTS_SUMMARY_HOME = f"{WORKSPACE_HOME}/results/240604/vlp_3090"
""">>> `${WORKSPACE_HOME}`/results/`${YYMMDD}`/vlp_3090"""
...
if __name__ == "__main__":
    ...
    - model
        - AdjustedBiasedCorrelationEstimator
            ...
            - Train
                ...
                - Func. Seq.: (adjust_score >> personalization)
                - DType. Seq.: V >> L >> P
    '''

    # Common Rules (Purchases)
    top_n_tags_list = [20]
    co_occur_items_list = [4]
    dv_score_list = [0.0]
    frob_norm_list = [1]
    score_iter_list = [3]
    score_learing_rate_list = [10 ** (-1 * 3)]
    score_generalization_list = [10 ** (-1 * 5)]
    weight_iter_list = [5]
    weight_learning_rate_list = [10 ** (-1 * 3)]
    ...
    # AdjustScore
    score_iter_list = [3]
    score_iter_list.extend([n for n in range(5, 25, 5)])
    score_learing_rate_list = [10 ** (-1 * n) for n in range(1, 5, 1)]
    # score_generalization_list = [0.1, 0.01]
    score_feedback_ratio_list = [0.1, 0.01]
    # Personalization
            ...
            for co_occur_items in co_occur_items_list:
                for score_iterations in score_iter_list:
                    for score_learning_rate in score_learing_rate_list:
                        for score_feedback_ratio in score_feedback_ratio_list:
