...
def build_model(
    ...
    _model: IPIRecModelSeries2 = None
    if dataset == None:
    ...
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}.bin",
        RESOURCES_DIR_HOME,
        IPIRecModelSeries2.__name__,
        fold_set_no,
    )
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        with open(file=_BIN_MODEL_FILE_PATH, mode="rb") as fin:
            _model: IPIRecModelSeries2 = pickle.load(fin)
            fin.close()
        # end : StreamReader()
    else:
        _model_params = IPIRecModelSeries2.create_models_parameters(
            top_n_tags=top_n_tags,
            co_occur_items_threshold=co_occur_items,
        )
        _model = IPIRecModelSeries2(
            dataset=dataset,
...
def build_estimator(
    ...
    _estimator: IPIRecEstimatorSeries2 = None
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}.bin",
        RESOURCES_DIR_HOME,
        IPIRecEstimatorSeries2.__name__,
        fold_set_no,
    )
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        with open(file=_BIN_MODEL_FILE_PATH, mode="rb") as fin:
            _estimator: IPIRecEstimatorSeries2 = pickle.load(fin)
            fin.close()
        # end : StreamReader()
    else:
        _model_params = IPIRecEstimatorSeries2.create_models_parameters(
        ...
        _estimator = IPIRecEstimatorSeries2(
            model=model,
            model_params=_model_params,
        )
...
def dump_estimator(
    file_path: str,
    estimator: IPIRecEstimatorSeries2,
) -> None:
...
def load_estimator(file_path: str) -> IPIRecEstimatorSeries2:
    _estimator: IPIRecEstimatorSeries2 = None
    if os.path.exists(file_path):
...
def recommendation(
    ...
    _estimator: IPIRecEstimatorSeries2 = load_estimator(binary_estimator_file_path)
    _fit_estimator: IPIRecEstimatorSeries2 = None
    _loss_list = list()
    ...
    while True:
        ...
        if min(_loss_list) < _L:
            ...
            if _fit_estimator != None:
                _estimator: IPIRecEstimatorSeries2 = _fit_estimator
            break
        if min(_loss_list) > _L:
            _fit_estimator: IPIRecEstimatorSeries2 = copy.deepcopy(_estimator)
