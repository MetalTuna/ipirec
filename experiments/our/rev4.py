...
def build_estimator(
    ...
    _estimator: IPIRecEstimatorSeries3 = None
    """
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}_{3}.bin",
        RESOURCES_DIR_HOME,
    ...
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        with open(_BIN_MODEL_FILE_PATH, "rb") as fin:
            _estimator: IPIRecEstimatorSeries3 = pickle.load(fin)
            fin.close()
        # end : StreamReader()
        return _estimator
    # end : if
    ...
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        ...
        _model_params = IPIRecEstimatorSeries3.create_models_parameters(
...
def dump_estimator(
    file_path: str,
    estimator: IPIRecEstimatorSeries3,
) -> None:
...
def load_estimator(file_path: str) -> IPIRecEstimatorSeries3:
    _estimator: IPIRecEstimatorSeries3 = None
    if os.path.exists(file_path):
...
def recommendation(
    ...
    _estimator: IPIRecEstimatorSeries3 = load_estimator(binary_estimator_file_path)
    _fit_estimator: IPIRecEstimatorSeries3 = None
    _loss_list = list()
    ...
    while True:
        ...
        if min(_loss_list) < _L:
            ...
            if _fit_estimator != None:
                _estimator: IPIRecEstimatorSeries3 = _fit_estimator
            break
        if min(_loss_list) > _L:
            # avoid Mem.leak.
            # if _fit_estimator != None:
            # del _fit_estimator
            # gc.collect()
            _fit_estimator: IPIRecEstimatorSeries3 = copy.deepcopy(_estimator)
