...
def main():
    ...
    POST_TRAIN_SEQ = [DecisionType.E_PURCHASE]
...
def build_model(
    ...
    _model: IPIRecModelSeries3 = None
    if dataset == None:
    ...
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}.bin",
        RESOURCES_DIR_HOME,
        IPIRecModelSeries3.__name__,
        fold_set_no,
    )
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        with open(file=_BIN_MODEL_FILE_PATH, mode="rb") as fin:
            _model: IPIRecModelSeries3 = pickle.load(fin)
            fin.close()
        # end : StreamReader()
    else:
        _model = IPIRecModelSeries3(dataset)
        _model.analysis()
...
def build_estimator(
    ...
    _estimator: IPIRecEstimatorSeries4 = None
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}_{3}",
        RESOURCES_DIR_HOME,
        IPIRecEstimatorSeries4.__name__,
        fold_set_no,
    ...
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        with open(_BIN_MODEL_FILE_PATH, "rb") as fin:
            _estimator: IPIRecEstimatorSeries4 = pickle.load(fin)
            fin.close()
        # end : StreamReader()
        return _estimator
    # end : if
    ...
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        ...
        _model_params = IPIRecEstimatorSeries4.create_models_parameters(
        ...
        _estimator = IPIRecEstimatorSeries4(
            model=model,
...
def dump_estimator(
    file_path: str,
    estimator: IPIRecEstimatorSeries4,
) -> None:
...
def load_estimator(file_path: str) -> IPIRecEstimatorSeries4:
    _estimator: IPIRecEstimatorSeries4 = None
    if os.path.exists(file_path):
...
def recommendation(
    ...
    _estimator: IPIRecEstimatorSeries4 = load_estimator(binary_estimator_file_path)
    _fit_estimator: IPIRecEstimatorSeries4 = None
    _loss_list = list()
    ...
    while True:
        ...
        if min(_loss_list) < _L:
            ...
            if _fit_estimator != None:
                _estimator: IPIRecEstimatorSeries4 = _fit_estimator
            break
        if min(_loss_list) > _L:
            _fit_estimator: IPIRecEstimatorSeries4 = copy.deepcopy(_estimator)
