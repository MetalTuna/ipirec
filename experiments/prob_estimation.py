...
def build_estimator(
    ...
    frob_norm: int = 1,
) -> IPIRecProbabilityEstimator:
    _estimator: IPIRecProbabilityEstimator = None
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}.bin",
        RESOURCES_DIR_HOME,
    ...
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        with open(file=_BIN_MODEL_FILE_PATH, mode="rb") as fin:
            _estimator: IPIRecProbabilityEstimator = pickle.load(fin)
            fin.close()
        # end : StreamReader()
    else:
        _model_params = IPIRecProbabilityEstimator.create_models_parameters(
        ...
        _estimator = IPIRecProbabilityEstimator(
            model=model,
            model_params=_model_params,
        )
        dump_estimator(
...
def load_estimator(file_path: str) -> IPIRecProbabilityEstimator:
    _estimator: IPIRecProbabilityEstimator = None
    if os.path.exists(file_path):
...
def recommendation(
    ...
    _estimator: IPIRecProbabilityEstimator = load_estimator(binary_estimator_file_path)
    _fit_estimator: IPIRecProbabilityEstimator = None
    _loss_list = list()
    ...
    while True:
        ...
        if min(_loss_list) < _L:
            ...
            if _fit_estimator != None:
                _estimator: IPIRecProbabilityEstimator = _fit_estimator
            break
        if min(_loss_list) > _L:
            # avoid Mem.leak.
            # if _fit_estimator != None:
            # del _fit_estimator
            # gc.collect()
            _fit_estimator: IPIRecProbabilityEstimator = copy.deepcopy(_estimator)
