...
if __name__ == "__main__":
    ...
    similarity_file_path = f"{dump_dir_path}/item_pcc_ml_like.bin"
    dataset_dir_path = f"{WORKSPACE_HOME}/data/ml"
    testset_file_path = f"{dataset_dir_path}/like_list.csv"
    results_summary_path = (
        f"{WORKSPACE_HOME}/results/item_cf_ml_TopN_like_IRMetrics.csv"
    )

    # 모델의 매개변수들
    top_n_conditions = [n for n in range(3, 41, 2)]
