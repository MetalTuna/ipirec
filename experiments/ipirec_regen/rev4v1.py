...
def build_estimator(
...
) -> IPIRecEstimatorRev3Ver1:
    _estimator: IPIRecEstimatorRev3Ver1 = None
    """
    _BIN_MODEL_FILE_PATH = str.format(
        "{0}/{1}/{2}_{3}.bin",
        RESOURCES_DIR_HOME,
        IPIRecEstimatorRev3Ver1.__name__,
        fold_set_no,
    ...
    if os.path.exists(_BIN_MODEL_FILE_PATH):
        ...
        _model_params = IPIRecEstimatorRev3Ver1.create_models_parameters(
        ...
        _estimator = IPIRecEstimatorRev3Ver1(
            model=model,
...
def dump_estimator(
    file_path: str,
    estimator: IPIRecEstimatorRev3Ver1,
) -> None:
...
def load_estimator(file_path: str) -> IPIRecEstimatorRev3Ver1:
    _estimator: IPIRecEstimatorRev3Ver1 = None
    if os.path.exists(file_path):
...
def recommendation(
    ...
    _estimator: IPIRecEstimatorRev3Ver1 = load_estimator(binary_estimator_file_path)
    _fit_estimator: IPIRecEstimatorRev3Ver1 = None
    _loss_list = list()
    ...
    while True:
        ...
        if min(_loss_list) < _L:
            ...
            if _fit_estimator != None:
                _estimator: IPIRecEstimatorRev3Ver1 = _fit_estimator
            break
        if min(_loss_list) > _L:
            # avoid Mem.leak.
            # if _fit_estimator != None:
            # del _fit_estimator
            # gc.collect()
            _fit_estimator: IPIRecEstimatorRev3Ver1 = copy.deepcopy(_estimator)
