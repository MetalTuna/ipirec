...
def main():
    ...
    if not os.path.exists(__TRAINED_EST_FILE_PATH):
    ...
    _estimator = load_estimator(__TRAINED_EST_FILE_PATH)
    ...
    if _recommender == None:
        ...
        dump_recommender(file_path=file_path, recommender=_recommender)
    ...
    for decision_type in [
        ...
        _evaluator.top_n_eval(
            top_n_conditions=_TOP_N_ITEMS_LIST,
        )
        IRMetricsDF: DataFrame = _evaluator.evlautions_summary_df()
        print(IRMetricsDF)
        for _, r in IRMetricsDF.iterrows():
            # Conditions Precision Recall F1-score Accuracy Hits TP FP FN TN
            _top_n = int(r["Conditions"])
            _f1_score = float(r["F1-score"])
            _hits = int(r["Hits"])
        # end : for (metrics)

        # Rec. Items T. Freq. Dist.
        _tags_freq_cos = CosineItemsTagsFreqAddPenalty()
        TagsFreqDist: float = _tags_freq_cos.tags_freq_distance(
            test_set=test_file_path,
            recommender=_recommender,
        )
        print(
            str.format(
                "TagsFreq.Dist. Adj.Cos. ({0}): {1}",
                DecisionType.to_str(decision_type),
                TagsFreqDist,
            )
        )
        ...
        # T. S. Dist.
        _scores_evaluator = TagsScoreRMSEEvaluator(
            recommender=_recommender,
            file_path=test_file_path,
        )
        _scores_evaluator.eval()
    # end : for (PositiveDecisionsType)
