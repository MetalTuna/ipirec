...
[수정일]
- 24.06.20 18:26 
    - 모델, 추론기에 학습관련 configParser가 추가됐음
    - 거리기반 모듈에서는 이를 사용하지 않으므로, 관련처리가 적용된 모듈로 대치됨.
        - 관련모듈: BaseDistanceModel, BaseDistanceEstimator
"""
...
from ..model import BaseRecommender, BaseDataSet, BaseModel, BaseTrain
...
class BaseEvaluator(metaclass=ABCMeta):
    ...
    def _append_config_(
        ...
        if isinstance(self._recommender._estimator, BaseTrain):
            ## cp config_inst >> self
            config: ConfigParser = copy.deepcopy(
                self._recommender._estimator._config_info,
            )
        else:
            ## Distance model
            config = ConfigParser()
            ...
            _section = "Model"
            _option = "name"
            _value = type(self.model).__name__
            ...
            _section = "Estimator"
            _option = "name"
            _value = type(self._recommender._estimator).__name__
            config.add_section(_section)
            config.set(
                section=_section,
                option=_option,
    ...
    def model(self) -> BaseModel:
        return self._recommender._estimator._model
