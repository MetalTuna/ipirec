...
if __name__ == "__main__":
    ...
    lf_scores_arr = np.zeros(shape=(__L, __F), dtype=np.float64)
    i = j = 0
    for x_name, x_idx in lname_to_idx_dict.items():
        for y_name, y_idx in fname_to_idx_dict.items():
            lf_scores_arr[i][j] = _tags_score[x_idx][y_idx]
            j += 1
        # end : for (|F|)
        i += 1
        j = 0
    ...
    _min = np.min(lf_scores_arr)
    _max = np.max(lf_scores_arr)

    lname_list = list(lname_to_idx_dict.keys())
    fname_list = list(fname_to_idx_dict.keys())
    ...
    with open(file=_file_path, mode="wt") as fout:
        ...
        for x_idx in range(__L):
            ...
            for y_idx in range(__F):
                fout.write(f"{lf_scores_arr[x_idx][y_idx]},")
            # end : for (|F|)
            fout.write("\n")
        # end : for (|L|)
        fout.close()
    ...
    ax = sns.heatmap(
        np.transpose(lf_scores_arr),
        vmin=_min,
        vmax=_max,
        center=np.median(
            lf_scores_arr[(lf_scores_arr != 1.0) & (lf_scores_arr != 0.0)]
        ),
        cmap="Grays",
        # cmap="RdBu",
        xticklabels=lname_list,
        yticklabels=fname_list,
    )
    plt.show()
    # """
