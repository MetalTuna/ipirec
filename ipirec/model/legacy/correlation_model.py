...
class CorrelationModel(BaseModel):
    ...
    def __init__(
        ...
        dataset: BaseDataSet,
        top_n_tags: int = 10,
        co_occur_items_threshold: int = 20,
        iterations_threshold: int = 100,
        # err_sub_threshold: float = 0.005,
    ...
    def __init__(
        ...
        # self.arr_cooccur_items_score: np.ndarray = None

        # self._set_model_params_(model_params=model_params)
        '''
        ## const args...
        self.CO_OCCUR_ITEMS_THRESHOLD = (
            co_occur_items_threshold if co_occur_items_threshold > 0 else 20
        )
        """임계 값: 두 태그에 공통으로 속하는 항목의 개수 (int)"""
        self.LEARNING_RATE = learning_rate
        """보정에 사용될 오차의 변화비율"""
        self.ITER_THRESHOLD = iterations_threshold
        """반복에 대한 탈출조건"""
        # self.ERR_SUB_THRESHOLD = err_sub_threshold
        # """오차변화에 대한 탈출조건"""
        self.TOP_N_TAGS = (
            1000 if top_n_tags > 1000 else (10 if top_n_tags < 0 else top_n_tags)
        )
        """사용자가 의사결정한 상위 n개의 태그선별에 사용되는 변수"""
        '''
    ...
    def _preprocess_(self) -> None:
        ### 태그점수 계산을 위한 준비단계
        print("CorrelationModel.preprocess()")
    ...
    def _process_(self) -> None:
        ### 주 처리 (의사결정의 전이와 점수보정)
        # 봤다로 좋아요 추정하고, 잘 맞출 수 있을 때까지 태그점수의 가중치를 수정(전역)
        # 태그점수에 위에서 구한 가중치를 가해서 태그점수를 보정
        # 좋아요를 잘 맞출 수 있을 때까지, 각 사용자별 태그점수 가중치를 구한다. (지역)
        # 위에서 구한 태그점수에 각 사용자들의 태그 가중치에 가해서, 각 사용자별 태그점수를 구한다.
        # 이거로 구매했다를 잘 맞추도록 가중치를 구한다.

        # 봤다에 따른 태그점수, 확률 계산
        self.__item_based_tags_corr__()
        self.__user_based_tags_corr__()
        self.__tags_score__()
        # self.__view_probability__()

    # end : protected override void process()

    def _postprocess_(self) -> None:
        ### 후처리
        # 봤다로 구한 태그점수로 봤다부터 잘 맞추게 태그점수를 보정
        return
    ...
    def __tags_score__(self) -> None:
        print("CorrelationModel.tags_score()")
        """
    ...
    def __user_based_tags_corr__(self) -> None:
        print("CorrelationModel.user_based_tags_corr()")
        """
        _file_path = self._dump_dir_path + "/pcc_user.bin"
        if self.IS_DEBUG_MODE:
            if os.path.exists(_file_path):
                with open(file=_file_path, mode="rb") as fin:
                    self._ub_pcc_co_occur_score = pickle.load(fin)
                    fin.close()
                # end : StreamReader()
                return
            # contains binary dump
        ...
        for x_idx in tqdm(
            ...
            for y_idx in range(x_idx + 1, self.tags_count, 1):
                ...
                # numer = 0.0 if math.isnan(numer) else numer
                # symmetric corr
    ...
    def __item_based_tags_corr__(self) -> None:
        print("CorrelationModel.item_based_tags_corr()")
        """
        _file_path = self._dump_dir_path + "/pcc_item.bin"
        if self.IS_DEBUG_MODE:
            if os.path.exists(_file_path):
                with open(file=_file_path, mode="rb") as fin:
                    self._ib_pcc_co_occur_score = pickle.load(fin)
                    fin.close()
                # end : StreamReader()
                return
            # contains binary dump
        ...
        for x_idx in tqdm(
            ...
            for y_idx in range(x_idx + 1, self.tags_count, 1):
                ...
                # numer = numer / (math.sqrt(denom_x) * math.sqrt(denom_y))
                # numer = 0.0 if math.isnan(numer) else numer
                denom_x = math.sqrt(denom_x) * math.sqrt(denom_y)
                numer = 0.0 if denom_x == 0.0 else numer / denom_x
                # symmetric corr
    ...
    def __top_n_decision_tags__(self) -> None:
        ...
        print("CorrelationModel.top_n_decision_tags()")
    ...
    def __mean_freq_tags__(self) -> None:
        print("CorrelationModel.mean_freq_tags()")
