...
class BaseCorrelationEstimator(BaseTrain):
    ...
    def _preprocess_(self):
        ...
        for _ in tqdm(
            ...
            desc="IPIRec.AdjustTagsCorrelation()",
            ...
            for x_name in user.top_n_decision_tags_set:
                for y_name in item.tags_set:
                    ...
                    for tag_name in item.tags_set:
                        ...
                        # denom += np.fabs(arr_weights[x_idx][idx])
                    # end : for (T(i))
                    if denom == 0.0:
                        continue
                    ...
                    numer = (
                        1.0 if np.isinf(numer) else 0.0 if np.isnan(numer) else numer
                    )
                    # denom += weight - numer
    ...
    def _set_model_params_(self, model_params: dict) -> None:
        _conf = self._config_info
        _sec = "Estimator"
        if not _sec in _conf:
            _conf.add_section(_sec)
            _conf.set(_sec, "name", type(self).__name__)
        for k, v in model_params.items():
            _conf.set(_sec, k, str(v))
        # end : for (estimator_params)
        self._config_info = _conf

        # self.top_n_tags = model_params["top_n_tags"]
        # self.co_occur_items_freq = model_params["co_occur_items_freq"]
        ...
        # self.weight_iterations = model_params["weight_iterations"]
        # self.weight_learning_rate = model_params["weight_learning_rate"]
        # self.weight_generalization = model_params["weight_generalization"]
