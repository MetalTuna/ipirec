...
class IPIRecEstimatorRev3(AdjustedBiasedCorrelationEstimator):
    """
    - 요약:
        - 모델훈련이 재정의됨
    ...
    def train(
        ...
        _oracle_estimator: IPIRecEstimatorRev3 = None
        if BIN_DUMP_DIR_PATH != None:
            ...
            if os.path.exists(_prev_file_path):
                with open(_prev_file_path, "rb") as fin:
                    self: IPIRecEstimatorRev3 = pickle.load(fin)
                    fin.close()
                # end : StreamReader()
                return
            ...
            while True:
                ...
                if _min_hl > _hl:
                    if _oracle_estimator != None:
                        self: IPIRecEstimatorRev3 = _oracle_estimator
                    break
                if _min_hl < _hl:
                    _oracle_estimator: IPIRecEstimatorRev3 = copy.deepcopy(self)
                _HL.append(_hl)
            # end : while (!is_fit)
