...
class ELABasedRecommender(BaseRecommender):
    ...
    def __candidate_items__(self):
        ...
        for item_id in self.item_dict.keys():
            ...
            """
            liked_user_ids: set = item.dict_of_users_decision["like"]
            viewed_user_ids: set = item.dict_of_users_decision["view"].difference(
                liked_user_ids
            )
            for user_id in liked_user_ids:
                user: UserEntity = self.user_dict[user_id]
                candidate_item_ids.update(user.dict_of_decision_item_ids["like"])
            # end : for (liked_users)

            for user_id in viewed_user_ids:
                user: UserEntity = self.user_dict[user_id]
                user.candidate_item_ids_set.update(candidate_item_ids)
            # end : for (viewed_users)
            """
        # end : for (items)
